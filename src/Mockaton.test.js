...
import { API, AUTOGENERATED_500_COMMENT, DEFAULT_MOCK_COMMENT } from './ApiConstants.js'
...
describe('500', () => {
	it('autogenerated in-memory 500', async () => {
		await commander.select(`api/alternative${AUTOGENERATED_500_COMMENT}.GET.500.empty`)
	...
	it('toggles 500', async () => {
		...
		equal(await r0.json(), `api/basic${AUTOGENERATED_500_COMMENT}.GET.500.empty`)
		equal((await request(route)).status, 500)
		
		const r1 = await commander.toggle500('GET', route)
		equal(await r1.json(), file)
		equal((await request(route)).status, 200)
	})
})
...
describe('Registering', () => {
	const temp500 = `api/register${AUTOGENERATED_500_COMMENT}.PUT.500.empty`
