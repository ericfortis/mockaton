...
Mockaton is an HTTP mock server with the goal of improving
the frontend development and testing experience.

---

With Mockaton you don’t need to write code for wiring your mocks.
Instead, place your mocks in a directory and it will be scanned
...
There’s two options you can combine for having different mocks for a particular route:

### A. Adding comments in filenames
...
### B. Different response status code
For instance, you can have mocks with a `4xx` or `5xx` status code for triggering error
...
### Scrape Mocks from your Backend
...
## Dashboard
The dashboard lets you select a mock variant for a particular route, delaying responses,
and triggering an autogenerated `500` (Internal Server Error), among other features.

Nonetheless, there’s a programmatic API see **Commander&nbsp;API** below.
